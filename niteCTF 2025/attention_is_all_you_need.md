...
Delving into further detail,
This challenge gives us a Linux ELF64 program that asks for a flag, but it doesn’t check the flag like comparing strings. Instead, it uses a small “AI-like” scoring system that was meant to run on the GPU. The program only accepts input that is exactly 29 characters long and only uses characters from this set: abcdefghijklmnopqrstuvwxyz0123456789{}_. Inside the file, the model’s numbers (the “weights”) are saved in an encrypted form. The secret needed to decrypt them (a small 16-byte XOR key) is hidden inside the GPU code that’s also packed into the same program, in a section called .nv_fatbin which contains smaller embedded ELF files (the GPU “cubin” pieces). The solve works by reading the program as raw bytes, finding the GPU blob, searching for the embedded ELF headers internally (7F 45 4C 46), and pulling out the XOR key from a GPU constant section. Then it uses that key to decrypt the stored weights (XOR each 32-bit chunk, then treat the bits as float numbers), rebuilds the same scoring math on the CPU, and tries every possible character at each of the 29 positions (only 39 options each). Because the model is basically “memorizing” one correct answer, this quickly reveals the one string that scores high enough to pass, thus granting us the flag. 
Upon disassembly, we see that the XOR key is in .nv.constant3. Also utilised greadelf and gobjcopy to dump the CUDA fatbin section for initial analysis. 
